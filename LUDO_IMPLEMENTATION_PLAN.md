...
> [`ludo-dev/`](ludo-dev/) that the tests exercise. **648 assertions green**, including
...
```
node ludo-dev/verify-all.js     # 648 assertions across 9 suites, non-zero exit on failure
```
...
| **2026-08-08** | **The difficulty ladder is now visible and can be pinned** | *"The CPU is well in control of what rolls it needs. The algorithm nerfs my rolls."* Chased the dice claim to the end and found nothing: `ludoRollDice` is `1 + Math.floor(Math.random() * 6)`, takes no seat, has one call site, and nothing in the file reassigns `Math.random`. `initLudoGame` guards on `ludoAnimFrame` so no second loop can run, and `ludoSettleRoll` overwrites the tumble flicker, so the face shown is the face banked. Then tested the last mechanism that could produce the complaint without any bias — that the rules compound sixes (a 6 buys a die, a capture buys a sequence, a seat stuck in base rolls once and stops), so the leader would *see* more sixes at a fair rate. New `six-count-check.js` measures counts rather than rates: 88.5 vs 89.8 rolls and **14.68 vs 14.79 sixes per game**. Level. That hypothesis is dead too. **But the adaptive ladder is a real algorithm that targets the player, and describing it as a footnote was wrong.** Win >65% and `ludoDifficultyTier` promotes the CPU to `hard`, where `ludoScoreMove` docks 60 for landing anywhere reachable — measured player win rate ~80% easy, ~50% normal, ~40% hard, and 12–13% for a casual player against `hard`. The only notice is a four-letter chip. So `ludoDifficulty` (`adaptive` default, or pin `easy`/`normal`/`hard`) now sits in ⚙️ under 🎲 Ludo CPU, and the dice audit is a line in the same panel instead of a console call — an audit that needs a console is an audit nobody runs. XP still scales by tier (×0.6 / ×1.0 / ×1.35), so pinning `easy` costs XP and cannot be farmed. 648 assertions. |
| **2026-08-08** | **`reinsert.js` — the engine copy is no longer moved by hand** | Every previous insertion into `AttendanceTimeCheckerPlus.js` was manual, which is exactly how the parity assertion would eventually be satisfied by a copy nobody verified. `reinsert.js` reconstructs the block currently in the file from git, requires a single exact match, and exits non-zero without writing if it finds none or finds two — a partial match would corrupt 16k lines. |
...
| **2026-08-05** | **CPU difficulty does not, and cannot, affect anyone's dice** | Worth recording because it is a natural thing to suspect twice. The tier is consumed only by `ludoAIChooseMove` — which legal move the CPU picks — and never reaches `ludoRollDice`. Measured in `fairness-check.js`: the player's six rate across easy/normal/hard is 16.45% / 16.56% / 16.65% against a fair 16.67%, i.e. flat. The tier does swing the *outcome* hard (player wins 76% / 47% / 42%) and captures you more (1.16 / 1.71 / 1.98 tokens lost per game), and since a capture returns a token to base it very slightly raises time spent needing a six — but only from 8.2 to 9.0 base-locked turns across the whole range. **So "the CPU will drop to easy" is not a remedy for a six drought.** The only levers that touch that are `ludoFreeRelease` or a pity timer, both declined. |
